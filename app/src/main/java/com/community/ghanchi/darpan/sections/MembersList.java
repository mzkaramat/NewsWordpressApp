...
import com.community.ghanchi.darpan.adapter.MemberListAdapter;
import com.community.ghanchi.darpan.model.MemberModel;
...
//                    adapter.notifyDataSetChanged();
                    new SearchMember().execute(new String[]{SearchString.getText().toString()});
                }else{
                    Memberslist.clear();
                    for(int i = 0 ; i <TempMemberlist.size();i++){
                            Memberslist.add((TempMemberlist.get(i)));
            ...
            try {
                ...
                if (status == 200) {
                   ...
                   for(int i = 0 ; i < data.length;i++){
                       Memberslist.add(new MemberModel(data[i].split(":",-1)[1],data[i].split(":",-1)[0],data[i].split(":",-1)[2].trim(),data[i].split(":",-1)[3].trim()));
                   }
    ...
    class SearchMember extends AsyncTask<String, Void, String> {
        ...
        protected String doInBackground(String... params) {
            ...
            try {
                //------------------>>
                HttpGet httppost = new HttpGet(("http://ghanchidarpan.org/news/searchMember.php?searchMember=" +
                        encodeHTML(params[0]) ).replaceAll(" ", "%20") );
                HttpClient httpclient = new DefaultHttpClient();
                HttpResponse response = httpclient.execute(httppost);
...
//                    TempMemberlist = new ArrayList<>(Memberslist);
        ...
        protected void onPostExecute(String result) {
            ...
            View view = MembersList.this.getCurrentFocus();
            if (view != null) {
